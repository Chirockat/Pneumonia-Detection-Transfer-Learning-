...
for epoch in range(num_epochs):
    ...
    progress_bar = tqdm (train_loader, desc=f"Epoch {epoch + 1} Progress")
    ...
    print(f"Average loss (Error): {running_loss / len(train_loader):.4f}")


    # Zapisujemy same wagi do pliku z rozszerzeniem .pth (standard PyTorcha)
    torch.save(model.state_dict(), 'model_wages.pth')
